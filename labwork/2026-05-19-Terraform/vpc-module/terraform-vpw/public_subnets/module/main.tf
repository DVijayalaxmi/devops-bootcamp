...
resource "aws_subnet" "main-public-vpw" {
  ...
  tags = merge(var.tags, {
    Name = "vijayalaxmipw-11-pub-subnet-1"
  })
}

# Resource-2: Public Subnet2
resource "aws_subnet" "main-public-vpw-2" {
  vpc_id                  = data.aws_vpc.vpc-vpw.id
  ...
  tags = merge(var.tags, {
    Name = "vijayalaxmipw-11-pub-subnet-2"
  })
}
...
resource "aws_route_table_association" "main-public_rt_assoc-vpw-2" {
  subnet_id      = aws_subnet.main-public-vpw-2.id
  route_table_id = data.aws_route_table.public_rt-vpw.id
}
